...
#해석 : 우선 데이터에 NA값이 존재해서 NA값이 존재하는 행은 모두 계산에서 배제하기 위해 na.omit() 함수를 활용했다. 그 뒤 미세먼지와 초미세먼지에 대한 tibble을 따로 만들었다           먼저 group_by 함수를 이용해 각 구별로 구분짓고 summarise함수를 통해 각 구마다 최대값 중앙값 평균값을 구했다.
...
#해석 : 원본데이터에서 초미세먼지에 대한 정보를 제외하고 일시와 장소 미세먼지에대한 정보만 select후 미세먼지가 높은순으로 정렬 한뒤 첫번째 값만 출력했다.
#정답 : 2021-05-08 1:00 중구
a=data%>%select(Time,Division,Ultra_Particulates)%>%arrange(Ultra_Particulates)
View(a)
#해석 : 원본 데이터에서 미세먼지에 대한 정보를 제외하고 일시와 장소 초미세먼지에 대한 정보만 select한 후 초미세먼지를 기준으로 오름차순 정렬을 했다. 
#결과 : 초미세먼지 측정값의 최저치가 "1"인 일시와 지역이 너무 많다. View를 활용해 데이터를 확인한 결과 5754번의 일시가 최저치로 측정되었다.
